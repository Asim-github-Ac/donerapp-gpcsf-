...
public class AdminHallAdopter extends RecyclerView.Adapter<AdminHallAdopter.RecyclerHolder> {
    private Context mContext;
    private List<Hall>  mRestaurants;
    DatabaseReference databaseReference2;

    public AdminHallAdopter(Context mContext, List<Hall> mRestaurants) {
        this.mContext = mContext;
        this.mRestaurants = mRestaurants;
    }

    @Override
    public AdminHallAdopter.RecyclerHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new RecyclerHolder(LayoutInflater.from(mContext).inflate(R.layout.admin_req_layout,parent,false));

    }

    @Override
    public void onBindViewHolder(@NonNull AdminHallAdopter.RecyclerHolder holder, int position) {
    ...
    public class RecyclerHolder extends RecyclerView.ViewHolder {
        ...
        public RecyclerHolder(@NonNull View itemView) {
            ...
            databaseReference2 = FirebaseDatabase.getInstance().getReference("Hall");
            btnaccept.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    int position = getAdapterPosition();
                    Hall user = mRestaurants.get(position);
                    databaseReference2.child(user.gethID()).child("hStatus").setValue("1");
            ...
            btncancel.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    int position = getAdapterPosition();
                    Hall user = mRestaurants.get(position);
                    databaseReference2.child(user.gethID()).child("hStatus").setValue("0");
